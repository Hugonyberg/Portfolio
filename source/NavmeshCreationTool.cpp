...
void NavmeshCreationTool::HandleImGUI()
{
	...
	ImGui::Begin("Navmesh");
	{
		...
		if (myUseDebugRay) // Only show these options if Use Debug Ray is enabled
		{
			ImGui::Indent(); // Optional: Indent for better visual hierarchy
			...
			if (myFindClosestIntersectionOnly) // Only show these options if Use FindClosestIntersectionOnly is enabled
			{
				...
				if (ImGui::Button("Reset Path"))
				{
					...
					if (myPathFrom != DE::Vector3f{ 0.f,0.f,0.f } && myPathTo != DE::Vector3f{ 0.f,0.f,0.f } && !myShowPath)
					{
						...
						{
							// TODO : Implement pathfinding here and save to std::vector<DE::Vector3f> myPath;
...
void DreamEngine::NavmeshCreationTool::RenderNavmeshCentersAndConnections()
{
	...
	for (int i = 0; i < myPolyMesh.npolys; ++i)
	{
		...
		// Draw the center as a point (optional, you can draw it using a small sphere or marker)
		// You could add a small line to visualize it as a point if necessary
		lineToDraw.fromPosition = center;
		lineToDraw.toPosition = center + DreamEngine::Vector3f(0.f, 20.f, 0.f);  // Tiny offset to mark the center
		...
		// Now, iterate over the polygon's neighbors (stored after the vertices in the polys array)
		for (int j = 0; j < myPolyMesh.nvp; ++j)  // nvp is the number of vertices per polygon (3 for triangles)
		{
...
void DreamEngine::NavmeshCreationTool::RenderNavmeshDetailedFromRecastStruct()
{
	...
	// Check if the navmesh data is valid
	...
	for (int i = 0; i < myPolyMeshDetail.nmeshes; ++i)
	{
		...
		{
			//lineToDraw.color = { RandomFloat(),RandomFloat(), RandomFloat(), 1.f }; // TEST
			...
			for (int k = 0, kp = 2; k < 3; kp = k++)
			{
				...
				if (ef == 0) // Internal edge
				{
					...
					else
					{
						...
						lineToDraw.color = {
							1.0f,        // Red stays constant
							1.0f - h,    // Green fades out as it gets higher
							0.0f,        // Blue remains 0
...
void DreamEngine::NavmeshCreationTool::RenderNavmeshStruct()
{
	...
	// Iterate over the nodes in the navmesh and draw the centers and connections
	for (const Node& node : myNavmesh.nodes)
	{
		// Draw lines between the triangle's vertices
		...
		lineToDraw.toPosition = node.center + DreamEngine::Vector3f(0.f, 20.f, 0.f);  // Small vertical line
		lineToDraw.color = { 1.0f, 0.0f, 0.0f, 1.0f };  // Red color for centers
...
void DreamEngine::NavmeshCreationTool::RenderNavmeshGrid()
{
	...
	{
		// Get the line drawer for drawing the grid lines
		DreamEngine::LineDrawer& lineDrawer = DreamEngine::Engine::GetInstance()->GetGraphicsEngine().GetLineDrawer();
		DreamEngine::LinePrimitive lineToDraw;

		lineToDraw.color = { 0.0f, 0.5f, 0.1f, 1.0f };

		// Get grid information from the navmesh
		const auto& grid = myNavmesh.grid;

		// Draw vertical grid lines (columns)
		// We draw grid.cols+1 lines to cover all cell boundaries
		...
		// We draw grid.rows+1 lines to cover all cell boundaries
...
void DreamEngine::NavmeshCreationTool::Render()
{
	...
	if (myShowPolyMesh)
	{
		...
		//RenderNavmeshCentersAndConnections();
	}
...
void DreamEngine::NavmeshCreationTool::SetSoloMesh(std::shared_ptr<DreamEngine::ModelInstance> aSoloMesh)
{
	//mySoloMesh = aSoloMesh;
	aSoloMesh;
}
...
void DreamEngine::NavmeshCreationTool::BuildNavmesh() // SIMPLE MESH TEST
{
	...
	for (const auto& object : myMeshes) // To do: Change to our active scene objects. Give them a bool for Used In Navmesh.
	{
	...
	std::string name = "Path to parent"; // Maybe export navmesh directly to Engine or scene, instead of saving to obj file?
...

	{
		...
		//params.walkableHeight = myAgentHeight;
		//params.walkableRadius = myAgentRadius;
		//params.walkableClimb = myAgentMaxClimb;
		...
		//myDetourNavMesh = dtAllocNavMesh();
		//if (!myDetourNavMesh)
		//{
		//	dtFree(navData);
		...
		//status = myDetourNavMesh->init(navData, navDataSize, DT_TILE_FREE_DATA);
		...
		//}
		//
		//HandleNavmeshStructFromDetourNavmesh();
		//// DETOUR NAVMESH TEST END
	}
...
// Function to load rcPolyMeshDetail from a binary file
...
void DreamEngine::NavmeshCreationTool::LoadDetailedNavmeshFromOBJ(std::string aPath)
{
	...
	while (std::getline(file, line))
	{
		...
		{  // Face line
			...
			if (faceIndices.size() == 3) {  // Ensure the face is a triangle
				faces.push_back(faceIndices);
			}
		}
	}
...
void DreamEngine::NavmeshCreationTool::HandleNavmeshStruct(bool shouldInvertZ)
{
	...
	float xOffset = myDrawXOffset;  // X offset for drawing above ground
	float yOffset = myDrawYOffset;  // Y offset for drawing above ground
	float zOffset = myDrawZOffset;  // Z offset for drawing above ground
	...
	for (int i = 0; i < myPolyMesh.npolys; ++i)
	{
		...
		for (int j = 0; j < myPolyMesh.nvp; ++j)  // nvp is the number of vertices per polygon (3 for triangles)
		{
			...
			//if (neighbor != RC_MESH_NULL_IDX && neighbor < myPolyMesh.npolys)  // If a valid neighbor exists
			//{
			//	// Use the index of the neighbor's node, not the raw poly index
...
void DreamEngine::NavmeshCreationTool::PopulateDetailedNavmeshGrid()
{
	...
	// 1. Calculate the overall 2D bounds for the navmesh based on its vertices:
	...
	// 2. Calculate grid dimensions based on cell width and height:
	...
	// 3. Populate the grid with triangle (or node) indices:
	for (size_t i = 0; i < myDetailedNavmesh.nodes.size(); ++i)
	{
		...
		// Use an unordered_set to avoid adding the same cell multiple times.
		...
		//// Compute the triangle's bounding box in 2D:
		//DE::Vector2f triMin, triMax;
		//bool firstTriVert = true;
		//for (int idx : node.indices)
		//{
		//	const auto& v = myDetailedNavmesh.meshVertices[idx];
		//	DE::Vector2f v2d(v.x, v.z);
		//	if (firstTriVert)
		//	{
		//		triMin = v2d;
		//		triMax = v2d;
		...
		//	else
		//	{
		//		triMin.x = std::min(triMin.x, v2d.x);
		//		triMin.y = std::min(triMin.y, v2d.y);
		//		triMax.x = std::max(triMax.x, v2d.x);
		//		triMax.y = std::max(triMax.y, v2d.y);
		//	}
		//}

		//// Determine which cells this triangle covers.
		//int startCol = static_cast<int>((triMin.x - minBounds.x) / navmeshGrid.cellWidth);
		//int startRow = static_cast<int>((triMin.y - minBounds.y) / navmeshGrid.cellHeight);
		//int endCol = static_cast<int>((triMax.x - minBounds.x) / navmeshGrid.cellWidth);
		//int endRow = static_cast<int>((triMax.y - minBounds.y) / navmeshGrid.cellHeight);

		//// Clamp indices to grid boundaries 
		//startCol = std::max(0, std::min(startCol, navmeshGrid.cols - 1));
		//startRow = std::max(0, std::min(startRow, navmeshGrid.rows - 1));
		//endCol = std::max(0, std::min(endCol, navmeshGrid.cols - 1));
		//endRow = std::max(0, std::min(endRow, navmeshGrid.rows - 1));

		//// For each cell covered by the triangle, add the node's index.
		//for (int row = startRow; row <= endRow; ++row)
		//{
		//	for (int col = startCol; col <= endCol; ++col)
		//	{
		//		int cellIndex = row * navmeshGrid.cols + col;
		//		navmeshGrid.cells[cellIndex].push_back(static_cast<int>(i));
		//	}
		//}
	}
...
void DreamEngine::NavmeshCreationTool::PopulatePolyNavmeshGrid()
{
	...
	{
		// Project the 3D vertex to a 2D point (using x and z)
	...
	// Set these bounds in the grid structure:
	navmeshGrid.minBound = minBounds;
	navmeshGrid.maxBound = maxBounds;
	...
	// Prepare the grid cells (assuming cells is a vector of vectors, one per grid cell)
	navmeshGrid.cells.resize(navmeshGrid.rows * navmeshGrid.cols);
	...
	for (size_t i = 0; i < myNavmesh.nodes.size(); ++i)
	{
		...
		//for (int idx : node.indices)
		//{
		//	const auto& v = myNavmesh.meshVertices[idx];
		//	DE::Vector2f v2d(v.x, v.z);
		//	if (firstTriVert)
		//	{
		//		triMin = v2d;
		...
		//// Clamp indices to grid boundaries (in case of floating point rounding)
		//startCol = std::max(0, std::min(startCol, navmeshGrid.cols - 1));
		//startRow = std::max(0, std::min(startRow, navmeshGrid.rows - 1));
		//endCol = std::max(0, std::min(endCol, navmeshGrid.cols - 1));
		//endRow = std::max(0, std::min(endRow, navmeshGrid.rows - 1));

		//// For each cell covered by the triangle, add the triangle's (or node's) index.
		//for (int row = startRow; row <= endRow; ++row)
		//{
		//	for (int col = startCol; col <= endCol; ++col)
		//	{
	...
	// Finally, assign the populated grid to your navmesh.
...
std::vector<int> DreamEngine::NavmeshCreationTool::GetGridCellsForLine(const DE::Vector2f& start, const DE::Vector2f& end, const NavmeshGrid& grid)
{
	...
	// Calculate grid coordinates for the start and end points
...
//				vertCount++;
//
//				// Add the index of the vertex (relative to the current position in meshVertices)
//				myNavmesh.meshIndices.push_back(vertexOffset++);
//			}
//
...
//			newNode.center = center;
//
//			// Store indices for this node?s vertices
//			newNode.indices = { vertexOffset - 3, vertexOffset - 2, vertexOffset - 1 };
//
//			// Initialize connections to -1 (no connection)
//			newNode.connections = { -1, -1, -1 };
